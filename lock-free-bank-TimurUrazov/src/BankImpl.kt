...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun deposit(index: Int, amount: Long): Long { // First, validate method per-conditions
         ...
         */
        return helpOrUpdate(amount, index, false) { accountAmount: Long, am: Long ->
            accountAmount + am <= MAX_AMOUNT
        }
    }
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        return helpOrUpdate(amount, index, true) { accountAmount: Long, am: Long ->
            accountAmount - am >= 0
        }
    }

    private fun helpOrUpdate(amount: Long, index: Int, underflow: Boolean, checker: (accountAmount: Long, amount: Long) -> Boolean): Long {
        while (true) {
            val account = account(index)
            /*
             * If there is a pending operation on this account, then help to complete it first using
            ...
            check(checker(account.amount, amount)) { if (underflow) "Underflow" else "Overflow" }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val listIndexes = listOf(fromIndex, toIndex).sorted().map {
                Pair(it, acquire(it, this))
            }.filter { it.second != null }

            if (listIndexes.size == 2) {
                operation(listIndexes, listIndexes[0].first == toIndex)
            }

            listIndexes.forEach { release(it.first, this) }
        }

        private fun operation(indicesList: List<Pair<Int, AcquiredAccount?>>, swap: Boolean = false) {
            val from = (if (swap) indicesList[1].second else indicesList[0].second)!!
            val to = (if (swap) indicesList[0].second else indicesList[1].second)!!
            when {
