...
fun main() {
    val fileDescriptor = File(SOLUTION_FILE_NAME)
    val initial = State(1, 1, 0, 0)
    val edges = mutableSetOf<String>()
    dfs(initial, edges)
    fileDescriptor.bufferedWriter().use {
        it.write("Urazov Timur")
        it.newLine()
        for (edge in edges) {
            it.write(edge)
            it.newLine()
    ...
    fileDescriptor.readLines().forEachIndexed { index, line ->
...
fun dfs(state: State, edges: MutableSet<String>) {
    val nextSteps = mutableSetOf(state.moveP())
    state.moveQ()?.let { nextSteps.add(it) }
    for (step in nextSteps) {
        if (edges.add("$state -> $step")) {
            dfs(step, edges)
        }
    }
}
